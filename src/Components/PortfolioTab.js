...
function PortfolioTab(props) {
  ...
  let clickInstructionText =
    props.sideBarTabName !== "Press" ? (
  ...
  if (props.sideBarTabName === "All Startups") {
    ...
    renderCompanies.forEach((company) => {
      ...
      if (
        portfolioContent.largeFundraisingNames.includes(
    ...
    renderAllStartups = (
      <div>
        <h2>Acquisitions</h2>
        <div className="content-grid">{acquisitions}</div>
        <h2>YC</h2>
        <div className="content-grid">{yc}</div>
        <h2>Large Fundraising Rounds</h2>
        <div className="content-grid">{largeFundraising}</div>
        <h2>All Startups</h2>
        <div className="content-grid">{theRest}</div>
      </div>
    );
  }
  ...
  if (props.sideBarTabName === "Press") {
    renderPress = portfolioContent.pressList.map((article) => {
      return (
        <a href={article.address} target="_blank">
